	    ...
	    <div class="view_data">
	        <table class="wc100">
				<tbody>
					...
					<tr>
						<th>借书日期</th>
						<td><fmt:formatDate value="${model.create_time}" pattern="yyyy-MM-dd" /></td>
						<th>预期还书日期</th>
					...
					<tr>
						...
						<td colspan="3">
							<c:choose>
			            		<c:when test="${model.verify_state==1 }">待审核</c:when>
			            		<c:when test="${model.verify_state==2 }">通过</c:when>
			            		<c:when test="${model.verify_state==3 }">不通过</c:when>
			            	</c:choose>
						</td>
					</tr>
					<c:if test="${type==1 }">
