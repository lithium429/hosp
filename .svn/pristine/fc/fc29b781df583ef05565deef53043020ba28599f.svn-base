...
<div class="datalistbox" style="top:80px;">
	    ...
	    <thead>
	    	<tr>
		        ...
		        <th class="serial w5per">序号</th>
   				...
   				<th class="w10per">工号</th>
   				<th class="w10per">出生日期</th>
			...
			<c:forEach items="${list}" var="item" varStatus="vs">  
            	<tr >
					...
					<td  class="operations">
						...
						<a href="staff/update.do?id=${item.id}" class="update">修改</a>&nbsp;</c:if>
						<c:if test="${gdf:judgeRoleMenu(roleMenuList,'删除') }">
						<a href="staff/delete.do?id=${item.id}" class="delete">删除</a>&nbsp;</c:if>
						<c:if test="${gdf:judgeRoleMenu(roleMenuList,'查看') }">
						<a href="staff/view.do?id=${item.id}" class="view">查看</a></c:if>
					</td>
				</tr>            
			</c:forEach>
    ...
    <c:if test="${!empty list}">
		...
		<c:if test="${gdf:judgeRoleMenu(roleMenuList,'批量删除') }">
			<a class="btn" id="batch_delete" href="staff/batchdelete.do">批量删除</a></c:if>
		</div>
	</c:if>
