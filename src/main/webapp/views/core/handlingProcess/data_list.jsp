...
<div class="datalistbox" style="top:80px;">
	    ...
	    <thead>
	    	<tr>
		        ...
		        <th class="serial w5per">序号</th>
   				...
   				<th class="w12per">创建日期</th>
   				<th class="w10per">当前责任人</th>
   				<th class="w10per">办结日期</th>
   				<th class="w10per">关闭日期</th>
    	...
    	<tbody>
			<c:forEach items="${list}" var="item" varStatus="vs">  
            	<tr >
		            ...
		            <td>
		            	<fmt:formatDate value="${item.close_date}" pattern="yyyy-MM-dd" />	
		            </td>
		            <td>
	            		<c:choose>
					...
					<td  class="operations">
					<c:if test="${item.state==2 && gdf:judgeRoleMenu(roleMenuList,'关闭')}">
						<a href="handlingProcess/close.do?id=${item.id}" class="close">关闭</a></c:if>
    ...
    <c:if test="${!empty list}">
		...
		<c:if test="${gdf:judgeRoleMenu(roleMenuList,'批量关闭') }">
			<a class="btn" id="batch_close" href="handlingProcess/close.do">关闭</a></c:if>
		</div>
	</c:if>
