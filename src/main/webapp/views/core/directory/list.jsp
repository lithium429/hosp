...
<gd:Layout>
	...
	<div class="mb10 crumbs">
		<span class="icon ico_place"></span><b>您当前的位置：</b>文档管理 &gt; 我的文档
		<div class="model_top_fun dn" id="top_btn_div">
	...
	<div class="data_model colum_two clearfix">
		...
		<div class="rightbox" style="margin-left:320px;">
			<form id="spec_form" action="file/datalist.do" data-ajax="true" data-ajax-begin="loadBegin" data-ajax-failure="loadFailure" data-ajax-failure="loadComplete" data-ajax-loading="#loading_img" data-ajax-method="POST" data-ajax-mode="replace" data-ajax-update="#data_list" novalidate="novalidate"> 
	...
	<input type="hidden" id="show_share" value="${gdf:judgeRoleMenu(roleMenuList,'共享文件夹') }" />
	...
	var ztreeWrapper = (function(){
	    ...
	    function addHoverDom(treeId, treeNode) {
	        ...
	        if (treeNode.showShareBtn && $("#show_share").val()=="true") {
	            var aObj = $("#" + treeNode.tId + "_a");
	            if(!treeNode.isShare || treeNode.level == 0){ 
	            	if ($("#diyBtn_share_" + treeNode.id).length > 0) return;
		            var editStr = "<span  class='button share' title='共享' id='diyBtn_share_" + treeNode.id + "' onfocus='this.blur();' > </span>";
		            aObj.append(editStr);
		            var btn = $("#diyBtn_share_" + treeNode.id);
		            if (btn){ 
		            	btn.bind("click", function () {
		            		var idArray = [],
		            		childIds = getChildIds(treeNode, false);
		            		if(!util.isNull(treeNode.id)){
		            		...
		            		if(idArray.length === 0){
		            			informationMsg('没有可以共享的文件夹！')
		            			return false;
		            		}
		            		openPage({
			                	url: 'directory/share.do?ids=' + idArray.join(','),
			    				id: 'share_page',
			    				title: '共享文件夹',
			    				width: '650px',
			    				height: '300px',
	            ...
	            if(treeNode.isShare || treeNode.level == 0){
		            if ($("#diyBtn_unshare_" + treeNode.id).length > 0) return;
		            var editStr = "<span  class='button unshare' title='取消共享' id='diyBtn_unshare_" + treeNode.id + "' onfocus='this.blur();' > </span>";
		            aObj.append(editStr);
		            var btn = $("#diyBtn_unshare_" + treeNode.id);
		            if (btn){ 
		            	btn.bind("click", function () {
		            		var idArray = [],
		            		childIds = getChildIds(treeNode, true);
		            		if(!util.isNull(treeNode.id)){
		            			idArray.push(treeNode.id);
		            		}
		            		if(!util.isEmpty(childIds)){
		            		...
		            		if(idArray.length === 0){
		            			informationMsg('没有可以取消共享的文件夹！')
		            			return false;
		            		}
			                $.ajax({
			                	 type: 'POST',
					             dataType: 'json',
					             url: 'directory/unshare.do',
	    ...
	    function removeHoverDom(treeId, treeNode) {
	        ...
	        $("#diyBtn_share_" + treeNode.id).unbind().remove();
	        $("#diyBtn_unshare_" + treeNode.id).unbind().remove();
	    };
	      ...
	      //修改
	        if (!treeNode["isAdd"]) {
	            ...
	            $.ajax({
	                ...
	                data: { name: newName, parent_id: parentId, parent_ids: parentIds, module_type: 1, layer: Number(layer)},
	                cache: false,
	                async: false,
	                success: function (result) {
	                    if (result.flag) {
	...
	var editorDoc = KindEditor.editor({
        ...
        extraFileUploadParams: { directory_id: $('#directory_id'), module_type: 1 },
        //选择文件后回调的函数
    ...
    $(function () {
		...
		$('#btn_add').live('click',function(){
			...
			openPage({
				url: url + '?moduleType=1&directoryId=' + directoryId,
				id: 'add_page',
				title: '新建文件',
				width: '650px',
				height: '200px'
