		    ...
		    <thead>
		    	<tr>
			        ...
			        <th class="serial w5per">序号</th>
	   				<th class="w35per">短信内容</th>
	   				<th class="w8per">发信人</th>
	   				<th class="w12per">收信人</th>
	   				<th class="w12per">发送时间</th>
	   				<th class="w6per">短信条数</th>
	    	...
	    	<tbody>
				<c:forEach items="${list}" var="item" varStatus="vs">  
	            	<tr >
			            ...
			            <td>
			            	<c:choose>
			            		<c:when test="${item.state==1 }">成功</c:when>
			            		<c:when test="${item.state==2 }">失败</c:when>
			            	</c:choose>
			            </td>
						<td  class="operations">
						<c:if test="${type==0 && item.state==2 && gdf:judgeRoleMenu(roleMenuList,'重新发送') }">
							<a href="sms/send_again.do?id=${item.id}" class="send_again">重新发送</a>&nbsp;</c:if>
						<c:if test="${gdf:judgeRoleMenu(roleMenuList,'查看') }">
							<a href="sms/view.do?id=${item.id}" class="view">查看</a></c:if>
						</td>
					</tr>            
				</c:forEach>
...
<c:if test="${!empty list}">
	...
	<div class="bot_fun" style="width:300px;">
		...
		<c:if test="${type==0 && gdf:judgeRoleMenu(roleMenuList,'批量重新发送') }">
			<a class="btn" id="bath_send_again" href="sms/send_again.do">重新发送</a> </c:if>
	</div>
</c:if>
