...
<gd:PopLayout>
	...
	<gd:Navgation addr="排班管理 &gt; 总排班表"></gd:Navgation>
	<!-- S 添加信息 -->
	<div class="data_model colum_two clearfix">
		<div style="padding:10px 10px 5px 30px;">
			<select id="sel_dept_id">
				<c:forEach items="${deptList}" var="item" varStatus="vs">
					<option value="${item.id }" <c:if test="${deptId == item.id}">selected="selected"</c:if>>${gdf:buildTreeName(item.name, item.layer)}</option> 
				</c:forEach>
			</select>
			<select id="sel_year">
		...
		<div class="leftbox">
			<div class="data_model wc100 data_list_wrap dt_query">
				<form id="data_form" method="post" action="scheduling/all.do" data-ajax="true" data-ajax-begin="showTip" data-ajax-complete="hideTip" data-ajax-success="backToList" data-ajax-error="showError">
	...
	<script type="text/javascript">
		$(function(){
			...
			scheduling.init();
			ztreeWrapper.init();
			$('#sel_dept_id').change(scheduling.changeDept);
