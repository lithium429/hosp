...
<div class="data_model data_cont_wrap sms_info">
	...
	<form id="data_form" method="post" action="email/reply.do" data-ajax="true" data-ajax-begin="showTip" data-ajax-complete="hideTip" data-ajax-success="backToList" data-ajax-error="showError">	
	...
	<input type="hidden" name="reply_email_id" id="reply_email_id" value="${model.email_id }" />
		...
		<table class="view w70per left_box">
			<tbody>
				<tr>
					...
					<td>
						<div id="innerUser" class="dib inp_t user_dib mail_input">
							<span class="user_child user_child_sel${model.sender_id }" key_id="${model.sender_id }" key_name="${model.sender_name }">${model.sender_name }<em class="del_user">×</em></span>
						</div>		
					</td>
				</tr>
				<tr>
				...
				<tr>
					<th class="">主题：</th>
					<td>
						<input type="text" class="inp_t mail_input" name="subject" id="subject" value="回复：${model.subject }"/>	
					</td>
				</tr>
				<tr>
					<th class="">附件：</th>
					...
					<td>
						...
						<div id="content_div" >
							...
							<div style="border-bottom: #cccccc 1px solid; padding-bottom: 5px; padding-left: 15px; padding-right: 15px; background: #edf6db; font-size: 12px; padding-top: 5px"><span style="line-height: 16px"><b>发件人：</b>&nbsp;${model.sender_name }</span><br>
								...
								<c:if test="${model.is_send }"> <span style="line-height: 16px"><b>发送时间：</b>&nbsp;	<fmt:formatDate value="${model.create_time}" pattern="yyyy-MM-dd HH:mm:ss" /></span><br></c:if>
