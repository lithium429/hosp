...
<div class="datalistbox" style="top:80px;">
			...
			<c:forEach items="${list}" var="item" varStatus="vs">  
            	<tr >
					...
					<c:if test="${item.state==1 && gdf:judgeRoleMenu(roleMenuList,'修改')}">
						<a href="handlingProcess/update.do?id=${item.id}" class="update">修改</a>&nbsp;</c:if>
					<c:if test="${item.state==2 && my_id==item.user_id && gdf:judgeRoleMenu(roleMenuList,'处理') }">
    ...
    <c:if test="${!empty list}">
		...
		<c:if test="${gdf:judgeRoleMenu(roleMenuList,'批量删除') }">
			<a class="btn" id="batch_delete1" href="handlingProcess/batchdelete.do">批量删除</a></c:if>
		</div>
	</c:if>
