...
<div class="datalistbox" style="top:80px;">
			...
			<c:forEach items="${list}" var="item" varStatus="vs">  
            	<tr >
					...
					<td  class="operations">
						...
						<a href="adviceTopic/update.do?id=${item.id}" class="update">修改</a>&nbsp;</c:if>
						<c:if test="${gdf:judgeRoleMenu(roleMenuList,'删除') }">
						<a href="adviceTopic/delete.do?id=${item.id}" class="delete">删除</a>&nbsp;</c:if>
						<c:if test="${gdf:judgeRoleMenu(roleMenuList,'查看') }">
						<a href="adviceTopic/view.do?id=${item.id}" class="view">查看</a></c:if>
					</td>
				</tr>            
			</c:forEach>
    ...
    <c:if test="${!empty list}">
		...
		<c:if test="${gdf:judgeRoleMenu(roleMenuList,'批量删除') }">
			<a class="btn" id="batch_delete" href="adviceTopic/batchdelete.do">批量删除</a></c:if>
		</div>
	</c:if>
