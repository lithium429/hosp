...
<div class="datalistbox" style="top:80px;">
	    ...
	    <thead>
	    	<tr>
		        ...
		        <th class="serial w5per">序号</th>
   				<th class="w10per">会议室名称</th>
   				<th class="w12per">创建时间</th>
   				<th class="w8per">创建人</th>
   				<th class="w50per">备注</th>
			...
			<c:forEach items="${list}" var="item" varStatus="vs">  
            	<tr >
					...
					<td  class="operations">
						...
						<a href="meetingRoom/update.do?id=${item.id}" class="update">修改</a>&nbsp;</c:if>
						<c:if test="${gdf:judgeRoleMenu(roleMenuList,'删除') }">
						<a href="meetingRoom/delete.do?id=${item.id}" class="delete">删除</a>&nbsp;</c:if>
						<c:if test="${gdf:judgeRoleMenu(roleMenuList,'查看') }">
						<a href="meetingRoom/view.do?id=${item.id}" class="view">查看</a></c:if>
					</td>
				</tr>            
			</c:forEach>
    ...
    <c:if test="${!empty list}">
		...
		<c:if test="${gdf:judgeRoleMenu(roleMenuList,'批量删除') }">
			<a class="btn" id="batch_delete" href="meetingRoom/batchdelete.do">批量删除</a></c:if>
		</div>
	</c:if>
