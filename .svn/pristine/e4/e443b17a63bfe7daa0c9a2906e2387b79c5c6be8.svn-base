...
<%@ include file="/WEB-INF/common/taglibs.jsp"%>
	    	...
	    	<tbody>
				<c:forEach items="${list}" var="item" varStatus="vs">  
	            	<tr >
						...
						<td  class="operations">
							<a href="action/update.do?id=${item.id}" class="update">修改</a>&nbsp;
							<a href="action/delete.do?id=${item.id}" class="delete">删除</a>&nbsp;
							<a href="action/view.do?id=${item.id}" class="view">查看</a>
						</td>
					</tr>            
				</c:forEach>
	    ...
	    <c:if test="${!empty list}">
			...
			<div class="bot_fun" style="width:300px;">
				<a class="btn" id="batch_delete" href="action/batchdelete.do">批量删除</a>
			</div>
		</c:if>