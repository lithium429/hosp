...
<gd:PopLayout>
	...
	<jsp:body>
		...
		<div class="data_model data_cont_wrap pop_wrap"> 
			<form id="data_form" method="post" action="menu/update.do" data-ajax="true" data-ajax-begin="showTip" data-ajax-complete="hideTip" data-ajax-success="backToList" data-ajax-error="showError">
				...
				<table class="view wc100">
					<tbody>
						...
						<tr>
							...
							<td>
								<input type="text" class="inp_t" data-val="true"
								data-val-remote-url="menu/validateUrl.do" 
						...
						<tr>
							<th>是否在新窗口打开</th>
							<td>
								<label><input type="radio" value="true" class="is_open_new" <c:if test="${model.is_open_new }">checked="checked"</c:if> name="is_open_new"  />是</label>
								<label><input type="radio" value="false" class="is_open_new ml30" <c:if test="${!model.is_open_new }">checked="checked"</c:if> name="is_open_new"  />否</label>
							</td>
						</tr>
					</tbody>
					<tfoot>
