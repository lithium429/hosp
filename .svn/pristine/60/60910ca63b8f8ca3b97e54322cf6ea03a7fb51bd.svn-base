...
public class User extends BaseEntity {
	 ...
	 * 临时字段
	 */
	private java.lang.Boolean is_new;
	...
	public User() {
		/*
		 * this.state = 1; this.create_time = new Date();
		 */
	}
	...
	public User(String name, String password, String realName, String sex, String deptName, Boolean hasAddress) {
		this.name = name;
		this.password = password;
		this.real_name = realName;
		this.sex = sex.equals("男") ? 1 : 2;
	...
	public void entryptPassword_old() {
		...
		System.out.println(this.password_old);
		System.out.println(this.password_hid);
	...
	public java.lang.Boolean getIs_new() {
		return is_new;
	}

	public void setIs_new(java.lang.Boolean is_new) {
		this.is_new = is_new;
	}
	 ...
	 * @return boolean
	 */
	public boolean isExisted(List<User> userList, String name) {
		if (userList == null || userList.isEmpty() || StringUtils.isBlank(name)) {
			return false;
		}
	...
	/**
	 * @Description 根据真实姓名获取用户
	 * @param userList
	 * @param name
	 * @return boolean
	 */
	public User gainExistedByRealName(List<User> userList, String realName) {
		if (userList == null || userList.isEmpty() || StringUtils.isBlank(realName)) {
			return null;
		}

		for (User user : userList) {
			if (realName.equals(user.getReal_name())) {
				return user;
			}
		}
		return null;
