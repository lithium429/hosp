	...
	<tbody>
		<c:forEach items="${list}" var="item" varStatus="vs">
			<tr>
				...
				<td>${item.sort}</td>
				<td>
					<a href="menu/update.do?id=${item.id}" class="update">修改</a>
					<a href="menu/delete.do?id=${item.id}" class="delete">删除</a>
					<a href="menu/view.do?id=${item.id}" class="view">查看</a>
					<c:if test="${!empty item.parent_id}">
						<a href="action/list.do?menu_id=${item.id}" class="action">菜单动作</a></c:if>
				</td>
			</tr>
		</c:forEach>
...
<c:if test="${!empty list}">
	...
	<div class="bot_fun" style="width: 300px;">
		<a class="btn" id="batch_delete" href="menu/batchdelete.do">批量删除</a>
	</div>
</c:if>
<span class="btn btn_pub" style="position:fixed;bottom:16px;left:115px;"><a href="menu/add.do" id="btn_add_bottom">添加</a></span>
